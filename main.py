...
rawData = pd.read_csv('block_3.csv', encoding='windows-1251')
...
def init_func():
    for i in range(len(axes)):
        ...
        xLim = [min(coordData) - 0.1 * (max(coordData) - min(coordData)),
                max(coordData) + 0.1 * (max(coordData) - min(coordData))]
        yLim = [min(paramData) - 0.1 * (max(paramData) - min(paramData)),
                max(paramData) + 0.1 * (max(paramData) - min(paramData))]
...
def ini_draw(step=0):
    for i in range(len(parametersNames)):
        coordData = rawData[coordLabel]
        paramData = rawData[parametersNames[i]]
        plots.append(axes[i].plot(coordData[step * data_skip: (step + 1) * data_skip],
                                  paramData[step * data_skip: (step + 1) * data_skip], color='b'))
